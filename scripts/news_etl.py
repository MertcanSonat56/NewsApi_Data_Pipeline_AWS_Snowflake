...
def runner():
    ...
    base_url = "https://newsapi.org/v2/everything?q={}&from={}&sortBy=popularity&apiKey={}&language=en"
    print(base_url)
    ...
    for i in d['articles']:
        ...
        df = pd.concat([df, pd.DataFrame({'newsTitle': newsTitle, 'timestamp': timestamp, 
                                          'url_source': url_source, 'content': trimmed_part,
                                          'source': source, 'author': author,
                                          'urlToImage': urlToImage})], ignore_index=True)
